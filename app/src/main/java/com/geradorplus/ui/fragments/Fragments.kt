...
import com.geradorplus.ui.activities.AdminActivity
...
class HomeFragment : Fragment() {
    ...
    private fun setupMoviesRecycler() {
        val adapter = ContentCardAdapter(emptyList(), viewModel.getImageUrl("")) { content ->
            openBannerEditor(content.id, content.isMovie())
        }
    ...
    private fun setupSeriesRecycler() {
        val adapter = ContentCardAdapter(emptyList(), viewModel.getImageUrl("")) { content ->
            openBannerEditor(content.id, false)
        }
    ...
    private fun openBannerEditor(tmdbId: Int, isMovie: Boolean) {
        startActivity(Intent(requireContext(), BannerEditorActivity::class.java).apply {
...
class SearchFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = SearchResultAdapter(viewModel.getImageUrl("")) { content ->
    ...
    private fun setupSearch() {
        binding.searchView.setOnQueryTextListener(object : androidx.appcompat.widget.SearchView.OnQueryTextListener {
            override fun onQueryTextSubmit(query: String?): Boolean {
                query?.let { viewModel.searchContent(it, currentSearchType) }
                return true
            }
    ...
    private fun setupTypeFilter() {
        binding.chipAll.setOnClickListener { currentSearchType = SearchType.ALL }
        binding.chipMovies.setOnClickListener { currentSearchType = SearchType.MOVIE }
        binding.chipSeries.setOnClickListener { currentSearchType = SearchType.SERIES }
    }
        ...
        if (result.resultCode == android.app.Activity.RESULT_OK) {
            result.data?.data?.let { uri ->
                ...
                path?.let {
                    ...
                    com.bumptech.glide.Glide.with(this).load(it).into(binding.ivLogo)
                }
            }
        }
    }
    ...
    private fun showChangePasswordDialog() {
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton("Alterar") { _, _ ->
                ...
                if (newPass == confirm) {
                    profileViewModel.changePassword("", newPass)
                } else {
...
class AdminDashboardFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adminViewModel.getUsers()?.observe(viewLifecycleOwner) { users ->
            ...
            val active = users.count { !it.isExpired() && it.isActive }
            val expired = users.count { it.isExpired() }
            val trial = users.count { it.licenseType == LicenseType.TRIAL }

            binding.tvTotalUsers.text = total.toString()
            binding.tvActiveUsers.text = active.toString()
            binding.tvExpiredUsers.text = expired.toString()
            binding.tvTrialUsers.text = trial.toString()
        }

        binding.btnManageUsers.setOnClickListener {
            (activity as? AdminActivity)?.let {
...
class UsersFragment : Fragment() {
    ...
    private fun showCreateUserDialog() {
        ...
        val radioMonthly = view.findViewById<android.widget.RadioButton>(R.id.radioMonthly)
    ...
    private fun showRenewDialog(user: User) {
        ...
        AlertDialog.Builder(requireContext())
            .setTitle("Renovar Licença - ${user.username}")
            .setItems(options) { _, which ->
                adminViewModel.renewLicense(user.id, days[which])
            }
            .show()
    }
    ...
    private fun confirmDeleteUser(user: User) {
        AlertDialog.Builder(requireContext())
            ...
            .setMessage("Excluir \"${user.username}\"? Esta ação não pode ser desfeita.")
